...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, stage=1, block_id=1, expand=True, output2=False):
    ...
    if expand:
        # Expand
        x = TimeDistributed(KL.Conv2D(expansion * in_channels, kernel_size=1,
                      padding='same', use_bias=False, activation=None, name=name + '_expand_conv'))(x)
        x = TimeDistributed(KL.BatchNormalization(epsilon=1e-3,
                                  momentum=0.999, name=name + '_expand_bn'))(x)
        x = TimeDistributed(KL.ReLU(6., name=name + '_expand_relu'))(x)

    out2 = x

    # Depthwise
    if stride == 2:
        x = TimeDistributed(KL.ZeroPadding2D(padding=correct_pad(K, x, 3),
                             name=name + '_dw_pad'))(x)
    x = TimeDistributed(KL.DepthwiseConv2D(kernel_size=3, strides=stride,
    ...
    x = TimeDistributed(KL.ReLU(6., name=name + '_dw_relu'))(x)

    # Project
    x = TimeDistributed(KL.Conv2D(pointwise_filters, kernel_size=1,
                  padding='same', use_bias=False, activation=None, name=name + '_project_conv'))(x)
    x = TimeDistributed(KL.BatchNormalization(
        epsilon=1e-3, momentum=0.999, name=name + '_project_bn'))(x)

    if in_channels == pointwise_filters and stride == 1:#testing here 
...
def _followed_down_sample_block(inputs, conv_out_channel, sep_out_channel, id):
    name = 'ssd_{}'.format(id)
    x = TimeDistributed(KL.Conv2D(conv_out_channel, kernel_size=1, padding='same', use_bias=False,
                  activation=None, name=name + '_conv'))(inputs)
    x = TimeDistributed(KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_conv_bn'))(x)
    x = TimeDistributed(KL.ReLU(6., name=name + '_conv_relu'))(x)

    x = TimeDistributed(KL.ZeroPadding2D(padding=correct_pad(K, x, 3), name=name + '_dw_pad'))(x)
    x = TimeDistributed(KL.DepthwiseConv2D(kernel_size=3, strides=2,
                           activation=None, use_bias=False, padding='valid', name=name + '_dw_conv'))(x)
    x = TimeDistributed(KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_dw_bn'))(x)
    x = TimeDistributed(KL.ReLU(6., name=name + '_dw_relu'))(x)

    x = TimeDistributed(KL.Conv2D(sep_out_channel, kernel_size=1, padding='same', use_bias=False,
                  activation=None, name=name + '_conv2'))(x)
    x = TimeDistributed(KL.BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_conv2_bn'))(x)
    x = TimeDistributed(KL.ReLU(6., name=name + '_conv2_relu'))(x)

    return x


def dw_sub_block(inputs, stride, alpha, filters, stage=1, block_id=1, output2=False):
    in_channels = K.int_shape(inputs)[-1]
    pointwise_conv_filters = int(filters * alpha)
    pointwise_filters = _make_divisible(pointwise_conv_filters, 8)
    x = inputs
...
def mobilenet_v2_ssdlite(input_image, sub):
    ...
    # stage subtraction fea9087ture map extraction
    # 300*300*3 -> 150*150*16
    sub = TimeDistributed(KL.ZeroPadding2D(padding=correct_pad(K, sub, 3),
                         name='sub_stage1_block1_pad'))(sub)
    sub = TimeDistributed(KL.Conv2D(first_block_filters,
                  kernel_size=3, strides=(2, 2),
                  padding='valid', use_bias=False,
                  name='sub_stage1_block1_conv'))(sub)
    sub = TimeDistributed(KL.BatchNormalization(
        epsilon=1e-3, momentum=0.999, name='sub_stage1_block1_bn'))(sub)
    sub = TimeDistributed(KL.ReLU(6., name='sub_stage1_block1_relu'))(sub)
    ...
    # stage1
    #use timeDistributed wrapper
    x = TimeDistributed(KL.ZeroPadding2D(padding=correct_pad(K, input_image, 3),
                         name='bbn_stage1_block1_pad'))(input_image)
    x = TimeDistributed(KL.Conv2D(first_block_filters,
                  kernel_size=3, strides=(2, 2),
                  padding='valid', use_bias=False,
                  name='bbn_stage1_block1_conv'))(x)
    x = TimeDistributed(KL.BatchNormalization(
        epsilon=1e-3, momentum=0.999, name='bbn_stage1_block1_bn'))(x)
    x = TimeDistributed(KL.ReLU(6., name='bbn_stage1_block1_relu'))(x)
    ...
    sub = dw_sub_block(sub, filters=32, alpha=alpha, stride=2, stage=3, block_id=1)
    x = KL.Add(name='38_38_32stage3_add')([x, sub])
    ...
    x = ConvLSTM2D(filters=80, kernel_size=(3,3), padding='same',stateful='True',return_sequences='False')(x)

...
from models.graphs.test_Bottleneck_rnn_net import mobilenet_v2_ssdlite #this is the backbone with bottelneck Lstm

# below is origin
from models.graphs.baseline_and_bottlenecklstm_change2ConvLSTM_for_test import mobilenet_v2_ssdlite #this is the backbone with bottelneck Lstm
...
def predict_block(inputs, out_channel, sym, id):
    ...
    x = TimeDistributed(DepthwiseConv2D(kernel_size=3, strides=1,
    ...
    x = TimeDistributed(Conv2D(out_channel, kernel_size=1, padding='same', use_bias=False,
...
def mobilenet_v2_ssd(image_size,
    ...
    #test
    #x1 = Lambda(identity_layer, output_shape=(1,), name='identity_layer')(x)
    #x2 = Lambda(identity_layer, output_shape=(1,), name='identity_layer2')(x_sub)
    if not (divide_by_stddev is None):
        x1 = Lambda(input_stddev_normalization, output_shape=(None, img_height, img_width, img_channels),
                    name='input_stddev_normalization')(x1)

    links = mobilenet_v2_ssdlite(x)

    #link1_cls = predict_block(links[0], n_boxes[0] * n_classes, 'cls', 1)
    link2_cls = predict_block(links[1], n_boxes[0] * n_classes, 'cls', 2)
    link3_cls = predict_block(links[2], n_boxes[1] * n_classes, 'cls', 3)
    link4_cls = predict_block(links[3], n_boxes[2] * n_classes, 'cls', 4)
    link5_cls = predict_block(links[4], n_boxes[3] * n_classes, 'cls', 5)
    #link6_cls = predict_block(links[5], n_boxes[5] * n_classes, 'cls', 6)

    #link1_box = predict_block(links[0], n_boxes[0] * 4, 'box', 1)
    link2_box = predict_block(links[1], n_boxes[0] * 4, 'box', 2)
    link3_box = predict_block(links[2], n_boxes[1] * 4, 'box', 3)
    link4_box = predict_block(links[3], n_boxes[2] * 4, 'box', 4)
    link5_box = predict_block(links[4], n_boxes[3] * 4, 'box', 5)
    #link6_box = predict_block(links[5], n_boxes[5] * 4, 'box', 6)
    '''
    priorbox1 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[0], next_scale=scales[1],
    ...
    '''
    priorbox2 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[0], next_scale=scales[1],
                                aspect_ratios=aspect_ratios[0],
    ...
    priorbox3 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[1], next_scale=scales[2],
    ...
    priorbox4 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[2], next_scale=scales[3],
    ...
    priorbox5 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[3], next_scale=scales[4],
    ...
    '''
    priorbox6 = TimeDistributed(AnchorBoxes(img_height, img_width, this_scale=scales[5], next_scale=scales[6],
   ...
   #cls1_reshape = Reshape((-1, n_classes), name='ssd_cls1_reshape')(link1_cls)
    ...
    cls = Concatenate(axis=1, name='ssd_cls')(
            [cls2_reshape, cls3_reshape, cls4_reshape, cls5_reshape])

        #[cls1_reshape, cls2_reshape, cls3_reshape, cls4_reshape, cls5_reshape, cls6_reshape])

    box = Concatenate(axis=1, name='ssd_box')(
        [box2_reshape, box3_reshape, box4_reshape, box5_reshape])

    priorbox = Concatenate(axis=1, name='ssd_priorbox')(
        [priorbox2_reshape, priorbox3_reshape,
         priorbox4_reshape, priorbox5_reshape])
    ...
    if return_predictor_sizes:
        predictor_sizes = np.array(
            [cls[1]._keras_shape[1:3], cls[2]._keras_shape[1:3],
             cls[3]._keras_shape[1:3], cls[4]._keras_shape[1:3]])

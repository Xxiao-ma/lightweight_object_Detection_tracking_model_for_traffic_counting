...
from keras.layers import Input, Lambda, Activation, Conv2D, \
    DepthwiseConv2D, Reshape, Concatenate, BatchNormalization, ReLU, TimeDistributed
...
def predict_block(inputs, out_channel, sym, id,sparse_facor=sparse_factor,cap=[]):
    ...
    if cap==[]:
        ...
        x = BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_dw_bn',gamma_regularizer=l1(sparse_facor))(x)
        ...
        x = BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + 'conv2_bn',gamma_regularizer=l1(sparse_facor))(x)
    else:
        x = DepthwiseConv2D(kernel_size=3, strides=1,
                            activation=None, use_bias=False, padding='same', name=name + '_dw_conv')(inputs)
        x = BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_dw_bn',gamma_regularizer=l1(sparse_facor))(x)
        ...
        x = Conv2D(cap[1], kernel_size=1, padding='same', use_bias=False,
                    activation=None, name=name + 'conv2')(x)
        x = BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + 'conv2_bn',gamma_regularizer=l1(sparse_facor))(x)
...
def mobilenet_v2_ssd(image_size,
    ...
    #to leverage time series information, expand the input shape to (timesteps, h, w, c)
    ...
    if cap==[]:
        links = mobilenet_v2_ssdlite(x1,x2, sparse_facor=sparse_factor, cap=[])

        link1_cls = predict_block(links[0], n_boxes[0] * n_classes, 'cls', 1,sparse_facor=sparse_factor, cap=[])
        link2_cls = predict_block(links[1], n_boxes[1] * n_classes, 'cls', 2,sparse_facor=sparse_factor, cap=[])
        link3_cls = predict_block(links[2], n_boxes[2] * n_classes, 'cls', 3,sparse_facor=sparse_factor, cap=[])
        link4_cls = predict_block(links[3], n_boxes[3] * n_classes, 'cls', 4,sparse_facor=sparse_factor, cap=[])
        link5_cls = predict_block(links[4], n_boxes[4] * n_classes, 'cls', 5,sparse_facor=sparse_factor, cap=[])
        link6_cls = predict_block(links[5], n_boxes[5] * n_classes, 'cls', 6,sparse_facor=sparse_factor, cap=[])

        link1_box = predict_block(links[0], n_boxes[0] * 4, 'box', 1,sparse_facor=sparse_factor, cap=[])
        link2_box = predict_block(links[1], n_boxes[1] * 4, 'box', 2,sparse_facor=sparse_factor, cap=[])
        link3_box = predict_block(links[2], n_boxes[2] * 4, 'box', 3,sparse_facor=sparse_factor, cap=[])
        link4_box = predict_block(links[3], n_boxes[3] * 4, 'box', 4,sparse_facor=sparse_factor, cap=[])
        link5_box = predict_block(links[4], n_boxes[4] * 4, 'box', 5,sparse_facor=sparse_factor, cap=[])
        link6_box = predict_block(links[5], n_boxes[5] * 4, 'box', 6,sparse_facor=sparse_factor, cap=[])
    else:
        links = mobilenet_v2_ssdlite(x1,x2, sparse_facor=sparse_factor, cap=cap[:72])

        link1_cls = predict_block(links[0], n_boxes[0] * n_classes, 'cls', 1,sparse_facor=sparse_factor, cap=cap[72:74])
        link2_cls = predict_block(links[1], n_boxes[1] * n_classes, 'cls', 2,sparse_facor=sparse_factor, cap=cap[74:76])
        link3_cls = predict_block(links[2], n_boxes[2] * n_classes, 'cls', 3,sparse_facor=sparse_factor, cap=cap[76:78])
        link4_cls = predict_block(links[3], n_boxes[3] * n_classes, 'cls', 4,sparse_facor=sparse_factor, cap=cap[78:80])
        link5_cls = predict_block(links[4], n_boxes[4] * n_classes, 'cls', 5,sparse_facor=sparse_factor, cap=cap[80:82])
        link6_cls = predict_block(links[5], n_boxes[5] * n_classes, 'cls', 6,sparse_facor=sparse_factor, cap=cap[82:84])

        link1_box = predict_block(links[0], n_boxes[0] * 4, 'box', 1,sparse_facor=sparse_factor, cap=cap[84:86])
        link2_box = predict_block(links[1], n_boxes[1] * 4, 'box', 2,sparse_facor=sparse_factor, cap=cap[86:88])
        link3_box = predict_block(links[2], n_boxes[2] * 4, 'box', 3,sparse_facor=sparse_factor, cap=cap[88:90])
        link4_box = predict_block(links[3], n_boxes[3] * 4, 'box', 4,sparse_facor=sparse_factor, cap=cap[90:92])
        link5_box = predict_block(links[4], n_boxes[4] * 4, 'box', 5,sparse_facor=sparse_factor, cap=cap[92:94])
        link6_box = predict_block(links[5], n_boxes[5] * 4, 'box', 6,sparse_facor=sparse_factor, cap=cap[94:96])

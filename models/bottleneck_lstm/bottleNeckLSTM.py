...
'''
class BottleneckLSTM2DCell(Layer):
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        super(BottleneckLSTM2DCell, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.recurrent_dw_kernel_i = self.add_weight(
        ...
        self.recurrent_pw_kernel_i = self.add_weight(
        ...
        self.recurrent_dw_kernel_c = self.add_weight(
            shape=recurrent_dw_kernel_shape,
            initializer=self.recurrent_initializer,
            name='recurrent_dw_kernel',
            regularizer=self.recurrent_regularizer,
        ...
        self.recurrent_pw_kernel_c = self.add_weight(
            shape=recurrent_pw_kernel_shape,
            initializer=self.recurrent_initializer,
            name='recurrent_pw_kernel',
            regularizer=self.recurrent_regularizer,
        ...
        self.recurrent_dw_kernel_f = self.add_weight(
            shape=recurrent_dw_kernel_shape,
            initializer=self.recurrent_initializer,
            name='recurrent_dw_kernel',
            regularizer=self.recurrent_regularizer,
        ...
        self.recurrent_pw_kernel_f = self.add_weight(
            shape=recurrent_pw_kernel_shape,
            initializer=self.recurrent_initializer,
            name='recurrent_pw_kernel',
            regularizer=self.recurrent_regularizer,
        ...
        self.kernel_i_d_k = self.recurrent_dw_kernel_i
        self.kernel_c_d_k = self.recurrent_dw_kernel_c
        self.kernel_f_d_k = self.recurrent_dw_kernel_f
        self.kernel_o_d_k = self.recurrent_dw_kernel_o

        #pointwise kernel initialization
        self.kernel_i_p_k = self.recurrent_pw_kernel_i
        self.kernel_c_p_k = self.recurrent_pw_kernel_c
        self.kernel_f_p_k = self.recurrent_pw_kernel_f
        self.kernel_o_p_k = self.recurrent_pw_kernel_o

        '''
        self.kernel_i_d_k = self.recurrent_dw_kernel[:, :, :, :self.filters]
        self.kernel_c_d_k = self.recurrent_dw_kernel[:, :, :, self.filters: self.filters * 2]
        self.kernel_f_d_k = self.recurrent_dw_kernel[:, :, :, self.filters * 2: self.filters * 3]
        self.kernel_o_d_k = self.recurrent_dw_kernel[:, :, :, self.filters * 3: self.filters * 4]

        self.kernel_i_p_k = self.recurrent_pw_kernel[:, :, :, :self.filters]
        self.kernel_c_p_k = self.recurrent_pw_kernel[:, :, :, self.filters: self.filters * 2]
        self.kernel_f_p_k = self.recurrent_pw_kernel[:, :, :, self.filters * 2: self.filters * 3]
        self.kernel_o_p_k = self.recurrent_pw_kernel[:, :, :, self.filters * 3: self.filters * 4]
        '''
    ...
    def call(self, inputs, states, training=None):
        ...
        i = self.seprable_conv(bottlenecks, self.kernel_i_d_k, self.kernel_i_p_k)
        c = self.seprable_conv(bottlenecks, self.kernel_c_d_k, self.kernel_c_p_k)
        f = self.seprable_conv(bottlenecks, self.kernel_f_d_k, self.kernel_f_p_k, b=self.bias_f)
        o = self.seprable_conv(bottlenecks, self.kernel_o_d_k, self.kernel_o_p_k)

        # recurrent_activation is sigmoid 
        # activation is relu (later relu 6 should be tried)
        new_c = (c_tm1 * self.recurrent_activation(f) +
                self.recurrent_activation(i)*self.activation(c))
...
class BottleneckLSTM2D(ConvRNN2D):
    """Convolutional LSTM.
    ...
    def __init__(self, filters,
                 ...
                 recurrent_dropout=0.,
                 **kwargs):
        cell = BottleneckLSTM2DCell(filters=filters,
        ...
        super(BottleneckLSTM2D, self).__init__(cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        return super(BottleneckLSTM2D, self).call(inputs,
                                            mask=mask,
                                            training=training,

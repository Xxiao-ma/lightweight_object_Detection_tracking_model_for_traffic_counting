...
def predict_block(inputs, out_channel, sym, id):
    ...
    x = TimeDistributed(BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + '_dw_bn'))(x)
    x = TimeDistributed(ReLU(6., name=name + '_dw_relu'))(x)

    x = TimeDistributed(Conv2D(out_channel, kernel_size=1, padding='same', use_bias=False,
                  activation=None, name=name + 'conv2'))(x)
    x = TimeDistributed(BatchNormalization(epsilon=1e-3, momentum=0.999, name=name + 'conv2_bn'))(x)
    return x

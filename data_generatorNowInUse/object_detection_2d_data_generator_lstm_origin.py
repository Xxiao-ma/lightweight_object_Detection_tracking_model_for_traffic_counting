...
class DataGenerator:
    ...
    def generate(self,
        ...
        # test_indices = np.reshape(test_indices,(1079,10))
        # print('shape of reshaped data indice', test_indices.shape)
        # print(test_indices[0])
        # test_image_ids = self.image_ids
        # test_image_ids = np.reshape(test_image_ids,(1079,10))
        # shuffled_test = [test_indices, test_image_ids]
        # X, Y = sklearn.utils.shuffle(*shuffled_test)
        # #print(shuffled_test_indices.shape)
        # print(X[0])
        # print(X[1])
        # X = X.flatten()
        # print(X)

        self.shuffle_indices = self.dataset_indices

        if shuffle:
            self.shuffle_indices = np.reshape(self.shuffle_indices,(1079,10))
            test_image_ids = self.image_ids
            test_image_ids = np.reshape(test_image_ids,(1079,10))
            shuffled_test = [self.shuffle_indices, test_image_ids]
            self.shuffle_indices, Y = sklearn.utils.shuffle(*shuffled_test)
            self.shuffle_indices  = self.shuffle_indices.flatten()
        ...
        while True:
            ...
            #    files from disk.
            batch_indices = self.shuffle_indices[current:current+batch_size]#self.dataset_indices[current:current+batch_size]
            #print("batch_indices")
            #print(batch_indices)

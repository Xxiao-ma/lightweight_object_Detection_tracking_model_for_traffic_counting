...
class DataGenerator:
    ...
    # load data from selected hdf5 file
    ...
    def parse_xml(self,
        ...
        for images_dir, image_set_filename, annotations_dir in zip(images_dirs, image_set_filenames, annotations_dirs):
            ...
            else: it = image_ids# image_ids is the name of each pics in the set file

            # Loop over all images in this dataset.
            for image_id in it:

                filename = '{}'.format(image_id[3:]) + '.pgm'#改变为pgm
                self.filenames.append(os.path.join(images_dir, filename))

                if not annotations_dir is None:
                    # Parse the XML file for this image.
                    with open(os.path.join(annotations_dir, image_id[3:] + '.xml')) as f:
                    ...
                    ## delete all the img without objects inside in the parse phase to keep the batch size stable
                    # Because stateful LSTM need the batch size to be invariable
                    if len(objects) == 0:
                        self.filenames.pop()
    ...
    # save loaded data as hdf5 format to accelerate training process
    ...
    def generate(self,
        ...
        #############################################################################################
        print('shape of origin data indice',self.dataset_indices.shape)
        test_indices = self.dataset_indices
        print(self.dataset_indices[:10])
        test_indices = test_indices.reshape((len(self.dataset_indices)/10,10))
        print('shape of reshaped data indice', test_indices.shape)
        ...
        #print(len(self.dataset_indices))
        current = 0
        while True:
            batch_X_seq, batch_y_seq = [],[]
                
            # set origin batch as sequences
            batch_X, batch_y = [], []
            
            if current >= (self.dataset_size-batch_size):
                current = 0
            ...
            #print("current batch between:")
            #print(start)
            #print(end)
            #print(len(batch_indices))
            if self.image_ids[start][:1] != self.image_ids[end][:1]:
                #print('\n id of the last frame of previous scen and first frame of \n')
                #print(self.image_ids[start])
                #print(self.image_ids[end])
                current+= (batch_size-1)# directly jump to the start frame of the next scenery
                #print("### scenery changed ### and the current id is :")
                #print(current)
            else:
    ...
    # generate data of batch_size 
    def generate_wrong_one(self,
                 batch_size=32,
                 shuffle=False,
                 transformations=[],
                 label_encoder=None,
        ...
        # Generate mini batches.
        #############################################################################################
        seq_size, batch_size = batch_size, seq_size
        current = 0
        while True:
            batch_X_seq, batch_y_seq = [],[]
            sizeOfSeqBatch = 1
            for sizeOfSeqBatch in range(0,seq_size):

                # set origin batch as sequences
                batch_X, batch_y = [], []
                
                if current >= self.dataset_size or current:
                    current = 0
                ...
                # Compose the output.
                #########################################################################################
                batch_X_seq.append(batch_X)
                batch_y_seq.append(batch_y_encoded)


            batch_X_seq=np.array(batch_X_seq)
            batch_y_seq=np.array(batch_y_seq)
            #batch_X_seq shape: (batch_size, seq_size, image_shape )
            #batch_y_seq shape: (batch_size, seq_size, output_size)
            #print(batch_X_seq.shape)
            #print(batch_y_seq.shape)
            #print(batch_y_encoded.shape)
            ret = []
            if 'processed_images' in returns: ret.append(batch_X_seq)
            if 'encoded_labels' in returns: ret.append(batch_y_seq)
            #if 'encoded_labels' in returns: ret.append(batch_y_encoded)
            if 'matched_anchors' in returns: ret.append(batch_matched_anchors)
            if 'processed_labels' in returns: ret.append(batch_y_seq)
            if 'filenames' in returns: ret.append(batch_filenames)
            if 'image_ids' in returns: ret.append(batch_image_ids)
            if 'evaluation-neutral' in returns: ret.append(batch_eval_neutral)
            if 'inverse_transform' in returns: ret.append(batch_inverse_transforms)
